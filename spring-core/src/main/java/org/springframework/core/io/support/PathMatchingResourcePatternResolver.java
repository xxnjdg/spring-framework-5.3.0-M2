...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {// 以 "classpath*:" 开头
			...
			if (getPathMatcher().isPattern(locationPattern.substring(CLASSPATH_ALL_URL_PREFIX.length()))) {
				...
				return findPathMatchingResources(locationPattern);// 路径包含通配符
			}
			else {
			...
			if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {
				...
				return new Resource[] {getResourceLoader().getResource(locationPattern)};// 路径不包含通配符
			}
		}
	}
	...
	protected String determineRootDir(String location) {
		int prefixEnd = location.indexOf(':') + 1;// 找到冒号的后一位
		int rootDirEnd = location.length();// 根目录结束位置
